...
5. Set validation expectations by repository type: software or product development repositories usually need complete layered tests, model training or algorithm repositories usually need as few targeted tests as practical with minimal defensive code, and other repository types should decide based on real risk and workflow.
6. If you maintain a local clone of this source repository, treat `.ai/tracking.md`, `.ai/plans/`, and `.ai/decisions/` as optional local maintenance files for the source repository itself, not as public template assets to copy by default.
