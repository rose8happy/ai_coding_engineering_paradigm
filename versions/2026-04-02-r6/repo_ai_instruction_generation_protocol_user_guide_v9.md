...
For most repositories, the fastest practical workflow is:

1. Copy `repo_ai_instruction_generation_protocol_v9.md` into the **target repository**. Copy this user guide too only if you want extra context, prompt wording, or optional-layer examples.
2. Ask the agent to inspect the target repository first and generate that repository's own canonical AI instruction file plus only the supporting files that are clearly justified.
3. Let the agent infer as much as possible from repository evidence before you answer questions.
4. Use `AGENTS.md` only as the default fallback filename when the target repository has no stronger signal.
5. Let the agent finalize the output and delete the copied bootstrap files unless you explicitly want them kept.
...
If you are using this repository directly, treat the latest core reusable starter set in `.ai/templates/` as optional support material, not as a bundle to copy by default:
- `main-instruction.md`
- `tracking.md`
- `plan.md`
- `decision.md`

Most repositories can start from the protocol file alone and only reach for these starters if the agent decides they are useful.
Use them as starters, not as a bundle to copy mechanically.
...
For direct use, validation expectations in the main instruction starter should also reflect repository type. Software or product development repositories usually benefit from complete layered tests. Model training, algorithm, or experiment-heavy repositories usually benefit from as few targeted tests as practical, experiment or metric validation, and minimal defensive code unless stricter safeguards are explicitly required. Other repository types should set test depth and defensive-coding expectations from real operational risk and workflow.
