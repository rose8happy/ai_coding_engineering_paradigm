...
1. 把 `repo_ai_instruction_generation_protocol_v9.md` 复制到**目标仓库**。只有在你还需要更多背景说明、prompt wording 或可选层示例时，再一起复制这份 user guide。
2. 要求 agent 先检查目标仓库，再生成该仓库自己的 canonical AI 指令文件，以及只有在确有理由时才创建的支持文件。
3. 在你回答问题之前，先让 agent 尽可能从仓库证据中推断信息。
4. 只有在目标仓库没有更强信号时，才把 `AGENTS.md` 当默认回退文件名。
5. 让 agent 最终完成输出，并在你没有明确要求保留时删除复制进来的 bootstrap 文件。
...
如果你直接使用本仓库，请把 `.ai/templates/` 里的最新核心 starter 集视为可选支持材料，而不是默认整包复制清单：
- `main-instruction.md`
- `tracking.md`
- `plan.md`
- `decision.md`

大多数仓库只靠协议文件本身就可以开始，只有在 agent 判断这些层确实有价值时，才需要再使用这些 starter。
...
直接使用模板时，主指令 starter 里的验证预期也应按仓库类型区分。开发或产品类仓库通常更适合完整的分级测试。模型训练、算法或实验密集型仓库通常更适合尽量少做测试，只保留必要且有针对性的测试，并结合实验或指标验证；除非明确需要更严格的保障，否则应尽量减少防御性代码。其他类型仓库则应根据真实运行风险与工作流来设定测试深度和防御性代码预期。
