...
10. **Acceptance criteria are steering**
    ...
    - For larger or multi-agent tasks, the task contract may live in a separate `spec` / requirements document while the plan stays focused on execution.
...
- when helpful, separate the task contract (`spec` / requirements) from the execution plan so `WHAT` and `HOW` stay distinct
...
Typical output:
- a task plan document or existing planning area, sometimes paired with a separate `spec` / requirements document for complex tasks, with acceptance criteria stated upfront
...
- enabled support layers and where they live, if those layers are used
...
- If you start from a neutral main-instruction starter, adapt it to repository reality and rename it to the repository's canonical instruction filename.
- Do not turn the main instruction file into a task log or investigation dump.
...
When starter templates are available, select only the ones that correspond to layers the repository actually needs.
A common core starter set is:
- one neutral main instruction starter
- one compact current-status starter
- one plan starter with a completion contract
- one decision starter

An advanced optional starter may also include a `spec` / requirements document for long, ambiguous, review-heavy, or multi-agent tasks.
Treat it as an extension of planning rather than part of the default core set.

Do not copy the full set mechanically into every repository.
...
### Usually split a task contract into a separate `spec` / requirements document when:
- the task is long or ambiguous
- review quality depends on a crisp contract before execution
- multiple contributors or agents will hand off work
- keeping `WHAT` separate from `HOW` reduces iteration drift
...
7. draft the final AI instruction file or adapt a neutral main-instruction starter
8. select only the starter templates justified for this repository
9. ask only a short list of high-impact questions
10. finalize the file and create needed supporting files
11. delete copied bootstrap protocol files unless told otherwise

---

## 14. Optional Starter Snippets and Template Files
Use only when the repository needs them.
Adjust names and locations to fit the repository.
If a template repository provides corresponding standalone starter files, treat these snippets and those files as reusable starting points rather than mandatory outputs.
The default core set remains main instruction, tracking, plan, and decision starters; `spec` / requirements is an optional advanced planning starter.
Do not copy the whole set by default.
